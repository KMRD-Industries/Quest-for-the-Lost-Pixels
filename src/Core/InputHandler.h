...
struct KeyHash {
    std::size_t operator()(const sf::Keyboard::Key& key) const {
        return std::hash<int>()(static_cast<int>(key));
    }
};

struct InputTypeHash {
    std::size_t operator()(const InputType& input) const {
        return std::hash<int>()(static_cast<int>(input));
    }
};
...
class InputHandler
{
    ...
    std::unordered_map<sf::Keyboard::Key, InputType, KeyHash> m_keyToMapInput{{sf::Keyboard::Key::W, InputType::MoveUp},
