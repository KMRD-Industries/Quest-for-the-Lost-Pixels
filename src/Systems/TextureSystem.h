...
private:
    std::vector<std::string> texture_files{
        "CosmicLilacTiles", "Decorative", "DungeonWalls", "Jungle", "Graveyard", "SpecialBlocks", "AnimSlimes",
    };

    std::unordered_map<std::string, sf::Texture> textures;
    std::unordered_map<long, sf::IntRect> texture_map;
    std::unordered_map<std::string, long> texture_indexes;
    std::unordered_map<long, std::vector<long>> map_animations;
    long no_textures = 0;
...
public:
    ...
    std::vector<long> getAnimations(long);
    void loadTextures();
};
