...
void EndGameState::init()
{
    m_menuButton.init(ButtonDTO{std::string(ASSET_PATH) + "/ui/uiHigh.png",
                                ASSET_PATH + std::string("/ui/uiFont.ttf"), "Menu",
                                40.f, 2.f,
                                {620, 644}, {0, 387, 144, 42},
    ...
    m_restartButton.init(ButtonDTO{std::string(ASSET_PATH) + "/ui/uiHigh.png",
                                   ASSET_PATH + std::string("/ui/uiFont.ttf"), "Restart",
                                   40.f, 2.f,
                                   {1010, 644}, {0, 387, 144, 42},
                                   {144, 387, 144, 42}, {288, 387, 144, 42},
                                   {0, 0}, {1, 1}, [this]()
                                   {
                                       m_restartGame = true;
    ...
    m_winText.init(TextDTO{ASSET_PATH + std::string("/ui/uiFont.ttf"), "You won!!", 160, 2.f, {0.f, 0.f, 0.f, 1.f},
                           {680, 200}});
}
...
void EndGameState::update(const float deltaTime)
{
    if (m_returnToMenu)
    {
        m_stateChangeCallback(
            {MenuStateMachine::StateAction::Pop, MenuStateMachine::StateAction::Pop},
