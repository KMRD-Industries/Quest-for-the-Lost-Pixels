...
void DungeonGenerator::generateSidePath(const sidePathConfig &pathConfigToCheck) {
    ...
    std::vector<glm::ivec2> pathToSidePath{
        m_paths[pathConfig.startingPathName].begin(),
    ...
    while (true) {
        ...
        if (auto neighbor{
                getRandomNeighbor(current, inPath, visitedNeighbors, minPathLength, maxPathLength,
    ...
    const auto checkIfNeighborIsCorrect{
        [this, &current, &nodesInPath, &visitedNeighbors, &neighbors, &endPath, &maxL, &minL](const auto &direction) {
            ...
            if (insideGraph && isNotInOtherPath) {
                ...
                const auto nodeVisited{
                    visitedNeighbors.contains(current) &&
