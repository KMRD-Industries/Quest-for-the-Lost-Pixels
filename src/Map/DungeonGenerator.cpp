...
DungeonGenerator::DungeonGenerator(int height, int width) : m_height(height), m_width(width), gen(std::chrono::system_clock::now().time_since_epoch().count())
{
}

void DungeonGenerator::generateMainPath(int pathLength)
{
...
DungeonGenerator::NodesPath DungeonGenerator::getNodes()
{
	return m_nodeToPath;
}

DungeonGenerator::RoomCount DungeonGenerator::getCount()
{
	return m_roomCount;
}
...
void DungeonGenerator::generateMainPath(const PathConfig& pathConfig)
{
	...
	while (path.size() < pathConfig.pathLength)
	{
		...
		if (const auto neighbor{ getRandomNeighbor(current,inPath,visitedNeighbors,pathConfig.pathLength,pathConfig.pathLength + 1) })
		{
...
void DungeonGenerator::generateSidePath(const sidePathConfig& pathConfig)
{
	...
	std::vector<glm::ivec2> pathToSidePath{ m_paths[pathConfig.startingPathName].begin(), m_paths[pathConfig.startingPathName].end() };
	...
	while (true)
	{
		...
		auto neighbor{ getRandomNeighbor(current,inPath,visitedNeighbors,minPathLength,maxPathLength,pathConfig.endPathName) };
		if (neighbor && path.size() <= maxPathLength)
		{
			const auto nextNode{ neighbor.value() };
			path.push_back(nextNode);
			inPath.insert(nextNode);
...
void DungeonGenerator::makeLockAndKey()
{
	...
	const auto sortByValue{ [](const auto& a, const auto& b)
	{
	...
	while (true)
	{
		...
		if (m_locks.contains(placeToLock.first) || (m_nodeToPath[placeToLock.first] == "Main" && m_roomCount[placeToLock.first] < 2) || m_roomCount[placeToLock.first] < 2)
		{
			nodes.pop_front();
			continue;
		}
...
std::optional<glm::ivec2> DungeonGenerator::getRandomNeighbor(const glm::ivec2& current, const std::unordered_set<glm::ivec2>& nodesInPath, const std::unordered_map<glm::ivec2, std::unordered_set<glm::ivec2>>& visitedNeighbors, int minL, int maxL, const std::string& endPath)
{
	std::vector<glm::ivec2> neighbors;
	constexpr glm::ivec2 directions[]{ {0,1},{0,-1},{1,0},{-1,0} };
	const auto checkIfNeighborIsCorrect{ [this, &current, &nodesInPath, &visitedNeighbors, &neighbors, &endPath, &maxL, &minL](const auto& direction) {
		...
		if (insideGraph && isNotInOtherPath) {
			...
			const auto nodeVisited {visitedNeighbors.contains(current) && visitedNeighbors.at(current).contains(neighbor)};
	...
	if (!neighbors.empty())
	{
		const auto nextNodeIndex{ randInt(0, std::size(neighbors) - 1)(gen) };
